...
namespace KernelImpl.Noyau.Deroulement
{
    public class Combat : INotifyPropertyChanged
    {
        ...
        public XElement ToXml(JudoData DC)
        {
            ...
            xjudoka1.SetAttributeValue(ConstantXML.Combat_PointsGRCH, CalculeScoreGRCH(DC, participant1));
            ...
            xjudoka2.SetAttributeValue(ConstantXML.Combat_PointsGRCH, CalculeScoreGRCH(DC, participant2));
           ...
           EtatCombattantEnum etat1, EtatCombattantEnum etat2, JudoData DC)
        {
            ...
            if (this.vainqueur != null && this.vainqueur > 0)
            {
                ...
                {
                    p1.cumulPointsGRCH = p1.cumulPointsGRCH + this.CalculeScoreGRCH(DC, this.vainqueur);
            ...
            if (DC.competition.type == (int)CompetitionTypeEnum.Shiai)
            {
                ...
                if (p1 != null)
                {
                    ...
                    if (points <= 0 || count_combat >= 5)
                    {
                        ...
                        {
                            c.Save(0, 0, 0, 0, 0, 0, 0, EtatCombattantEnum.Normal, EtatCombattantEnum.Normal, DC);
                            updated_combats = updated_combats.Concat(c.Update(DC)).ToList();
                            //c.vainqueur = 0;
                            //updated_combats = updated_combats.Concat(c.Update(DC)).ToList();
                        }
